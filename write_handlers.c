	...
	int precision, int size)
{
	...
	if (width > 1)
	{
		...
		if (flags & MINUS)
			...
			return (write(1, &buffer[BUF - i - 1],
				width - 1) + write(1, &buffer[0],
				1));
	}
	...
	int flags, int width, int precision, int size)
{
	...
	int length = BUF - ind - 1, i = 0;
	char padd = ' ';

	UNUSED(is_negative);
	UNUSED(size);

	if (precision == 0 && ind == BUF - 2 && buffer[ind] == '0')
		return (0); /* printf(".0d", 0)  no char is printed */
	...
	int flags, char padd, char extra_c, int padd_start)
{
	...
	if (extra_c)
		buffer[--ind] = extra_c;
	return (write(1, &buffer[ind], BUF - ind - 1));
}
